...
if [[ $# -lt 2 ]]; then
  cat <<'EOF'
Usage: src/scripts/run_postprocess_mavedb_functional_classifications.sh <input-file> <output-file> [postprocess_mavedb_functional_classifications options...]

Examples:
  src/scripts/run_postprocess_mavedb_functional_classifications.sh data/cvfg/v13/cvfg_variants.11.tsv data/cvfg/v13/cvfg_variants.12.tsv
  src/scripts/run_postprocess_mavedb_functional_classifications.sh input.tsv output.tsv --variant-urn-col variant_urn
...
cmd+=(--rm postprocess-mavedb-functional-classifications "$input_in_container" "$output_in_container")
if [[ $# -gt 0 ]]; then
  cmd+=("$@")
fi
...
# step_12, which runs from a variant-annotation checkout.
