...
if [[ $# -lt 2 ]]; then
  cat <<'EOF'
Usage: src/scripts/run_merge_rna_score_columns.sh <input-file> <output-file> [merge_rna_score_columns options...]

Examples:
  src/scripts/run_merge_rna_score_columns.sh data/cvfg/v19/cvfg_variants.20.codes.tsv data/cvfg/v19/cvfg_variants.20.tsv
  src/scripts/run_merge_rna_score_columns.sh input.tsv output.tsv --target-column rna_score --source-column rna_score_d6
...
cmd+=(--rm merge-rna-score-columns "$input_in_container" "$output_in_container")
if [[ $# -gt 0 ]]; then
  cmd+=("$@")
fi
