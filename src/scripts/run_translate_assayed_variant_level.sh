...
if [[ $# -lt 2 ]]; then
  cat <<'EOF'
Usage: src/scripts/run_translate_assayed_variant_level.sh <input-file> <output-file> [translate_assayed_variant_level options...]

Examples:
  src/scripts/run_translate_assayed_variant_level.sh data/cvfg/v19/cvfg_variants.19.tsv data/cvfg/v19/cvfg_variants.20.codes.tsv
  src/scripts/run_translate_assayed_variant_level.sh input.tsv output.tsv --column assayed_variant_level
...
cmd+=(--rm translate-assayed-variant-level "$input_in_container" "$output_in_container")
if [[ $# -gt 0 ]]; then
  cmd+=("$@")
fi
...
# step_20, which runs from a variant-annotation checkout.
