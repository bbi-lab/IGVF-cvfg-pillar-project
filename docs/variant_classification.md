# Variant classification deduplication strategy

`Variant_Classification_analysis.ipynb` classifies variants scored by
MAVE assays, some of which measure effect at DNA (nucleotide, "nt")
resolution and some at protein (amino-acid, "aa") resolution (see
`nucleotide_or_aa` throughout the notebook and
[`docs/assay_priority_questions.md`](assay_priority_questions.md) for the
full background). When the same physical variant is scored by more than
one assay -- possibly a mix of nt- and aa-resolution ones -- the pipeline
must collapse that to a single representative record before computing
points and a classification. This doc records the decided deduplication
methodology and why it differs between variant categories.
...
| `conflicting_fxn_data` | This coordinate group has rows with opposite-sign `Fxn_points` across assays -- see [Handling conflicting evidence](#handling-conflicting-evidence) below |
...
The dedup strategies above resolve *which record to keep* when multiple
assays score the same variant. Separately, the notebook has two distinct
mechanisms for evidence that actively *disagrees* -- one upstream of dedup
that excludes the variant outright, and one downstream of it that flags
the result without excluding anything. They address different questions
and neither one is affected by the choice of dedup strategy.
...
## Special handling for `F9` and `TP53`

`F9` and `TP53` get one more piece of upstream special-casing, unrelated to
dedup strategy but relevant to understanding why they rarely (`TP53`) or
never (`F9`) show up needing a dedup decision at all: before any category
split, conflict check, or dedup runs, a single cell drops every `F9` and
`TP53` dataset except one designated survivor per gene:
...
above) even though the exact rationale isn't spelled out in the notebook
itself. `TP53_Fortuno_2021` doesn't fit that pattern as neatly (it isn't
obviously a sub-readout of another kept/removed `TP53` dataset), but it's
excluded by the same rule regardless.
...
for `assay_priority` in the [table above](#where-assay_priority_list-actually-decides-an-outcome)),
